...
from src.network import UNet16
from tqdm import tqdm
import skimage.io
from src.sliding_window import zero_pad, sliding_window
...
def crack_segmentation(images_path, out_return=False):
    print(os.getcwd())
    # model_path = 'C:/Users/napol/Desktop/Alma-Sistemi/ERA4CH/ERA4CH-Platform/backend/models/mse+topo/mse+topo_full_weights_50_3e-05_100.0_10.0.pt'  # mse+topo
    model_path = './models/mse+topo/mse+topo_full_weights_50_3e-05_100.0_10.0.pt'  # mse+topo
    print("changed paths")
    warnings.filterwarnings("ignore")
    # result_path = "C:/Users/napol/Desktop/Alma-Sistemi/ERA4CH/ERA4CH-Platform/backend/results/"
    result_path = "./results/"

    if not os.path.exists(result_path):
        os.mkdir(result_path)
        print("I made a dir in: ", os.getcwd())
        print("the other dirs here are:", os.listdir())
    ...
    image_names = [i.split(os.sep)[-1]
                   for i in glob.glob(os.path.join(images_path, '*.png'))]
    # image_names += [i.split(os.sep)[-1]
    #                 for i in glob.glob(os.path.join(images_path, '*.JPG'))]
    image_names += [i.split(os.sep)[-1]
                    for i in glob.glob(os.path.join(images_path, '*.jpg'))]

    # Dictionary to save binary predictions
    crack_prediction_bin = {}
    print("the list of imagenames is, ", image_names)
    ...
    for image_name in tqdm(image_names):
        ...
        for (x, y, window) in sliding_window(padded_image, stepSize=desired_size, windowSize=(desired_size, desired_size)):
            window_names.append(
                image_name[:-4] + "_{:d}".format(x) + "_{:d}".format(y))
            window = T.ToPILImage()(window)  # as PIL
            window = transform(window)
            windows.append(torch.reshape(
                window, [1, 3, desired_size, desired_size]))
        ...
        with torch.no_grad():
            for window, window_name in zip(windows, window_names):
                ...
                overlay_crack[y:y + desired_size,
                              x:x + desired_size] = binary_result
        ...
        prediction_rgb = np.zeros(
            (overlay_crack.shape[0], overlay_crack.shape[1], 3), dtype='uint8')
        ...
        overlayed_prediction = cv.addWeighted(
            image_file, 0.5, prediction_rgb, 1.0, 0)
        ...
        return os.path.abspath(result_path+overlay_name_save)
